...
//Code Here
let evenNumbers = mixedNumbers.filter((number, index, wholeArray) => {
  if (number % 2 == 0) {
    return number;
  }
});
...
//Code Here
let postTaxPrices = prices.map((number) => {
  return number * 1.07;
});
...
//Code Here
let totalPopulation = populations.reduce((runningTotal, element) => {
...
let bobsTotal = purchases.reduce((total, element) => {
  return total + (element.owner == "Bob" ? element.price : 0);
}, 0);
